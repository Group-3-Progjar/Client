...
class CreateRoomScene(Scene):
    def __init__(self, game, game_client):
        ...
        self.game_client.register_callback("START_GAME", self.startGameCallback)
        ...
        super().addEntity('cr_start_button4', Button(self, 'assets/btn_confirm.png', 400, 360, self.startGame))
    ...
    def startGame(self):
        self.game.client.send_request('START_GAME', {"username":self.game.username, "map_id": chosen_map, "players": players})

    def startGameCallback(self, data):
        if data["success"]:
            self.game.leaderboard_id = data["leaderboard_id"]
            self.game.changeScene('STARTGAME')